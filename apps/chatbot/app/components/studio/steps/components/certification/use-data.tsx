...
export const useData = () => {
  ...
  const changeOrderOfCertifications = (index1: number, index2: number) => {
    form.setValue(
      "certifications",
      form.getValues("certifications")?.map((project, index) => {
        if (index === index1) {
          return form.getValues("certifications")![index2];
        } else if (index === index2) {
          return form.getValues("certifications")![index1];
        }
        return project;
      }) || []
    );
